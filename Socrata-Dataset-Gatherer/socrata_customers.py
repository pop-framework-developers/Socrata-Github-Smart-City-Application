...
#insercion de la tupla solo si tiene una url en open_data_site_url
    ...
    #comprobamos si ya existe una tupla con ese open_data_site_url
        ...
        if id_customer is not None:
            try:  
                cur.execute('UPDATE  socrata_customer SET type=?,location_name=?,country=?,customer_name=?,api_dcat_url=?,api_data_url=?,longitude=?,latitude=? WHERE id_customer=?',(portal_type,location_name,country,customer_name,api_dcat_url,api_data_url,longitude,latitude,id_customer[0]))
            except: 
